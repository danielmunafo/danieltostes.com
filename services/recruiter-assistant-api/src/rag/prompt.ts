...
- When fit is weak (**${rec.maybeValidate}**, **${rec.weakFit}**, **${rec.skip}**): stay intellectually honest; do not inflate the recommendation; **${Sev.major}:** risks are appropriate when core must-haves are missing.
...
4) \`# ${H.mainRisks}\` — always. **3-4** bullets max. Each bullet **must** start with exactly one of: **${Sev.major}:**, **${Sev.moderate}:**, **${Sev.minor}:**, or **${Sev.minorToModerate}:** (bold label + colon + space + one concise risk). Use **${Sev.major}:** only for **${Ev.termMustHaveTable}** gaps that are **${Ev.termNotEvidencedTable}** or only **${Ev.termAdjacentTable}** and **role-defining**; do **not** use **${Sev.major}:** for **${Ev.termNiceToHaveTable}**-only omissions unless the JD makes them the job's core. Strong matches should skew **${Sev.moderate}:** / **${Sev.minor}:** / **${Sev.minorToModerate}:**. When the JD calls out **web security fundamentals** (auth, authorization, data protection, secure coding) as an expectation and excerpts do **not** show Daniel **owning** that scope end to end, use **${Sev.moderate}:** (not **${Sev.minor}:** / **${Sev.minorToModerate}:**) unless the brief already shows **${Ev.termDirectTable}** security ownership.
5) \`# ${H.whyNotHigher}\` — **always when Technical fit Y/10 has Y < 10.** **Omit this heading only when Y = 10.** One short paragraph: what prevents a higher score. For **8** or **9**: keep tone **positive** — explain why not **9/10** or **10/10**; group **${Sev.moderate}:**/**${Sev.minor}:** items as **validation** vs core (do **not** sound like a rejection; do **not** let optional/minor themes dominate). For **6** or **7** with a **role-defining missing stack band**, name that the score stops because **specific** JD stack / tenure / depth requirements are not evidenced even when **general** senior full-stack fit is strong. For weaker fits, you may use 2-4 sentences and name real caps honestly.
...
- Avoid global phrases like "Daniel is a strong match" without qualification when **${Sev.major}:** applies to a **role-defining stack band**; prefer the **split-lens** phrasing from **TONE BY FIT**.
